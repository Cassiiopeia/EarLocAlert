...
구현된 것: 도메인 로직(지오펜스 판정·오디오 경로·광고 빈도) · Drift 저장소 · 권한 온보딩 · 알림 발화/해제 + 알림 화면 · 전면광고 통합 · 위치 목록/등록/편집 · 플랫폼 권한 선언 · **백그라운드 감시** · **지도 홈·위치 선택 화면** · **알림음** · **진동 세기 조절** · **장소별 알림음·사용자 음원 등록** · **알림 화면 지도 카드** · **Pretendard 폰트**. 테스트 549건.
...
| **#142 알림 화면 지도 카드 검증** | 에뮬레이터에서 실제 발화·여백 탭 해제까지 확인. 이중 승격·헤드업 겹침은 고쳤다. **남은 것: 사람 손으로 급하게 눌러보기** |
