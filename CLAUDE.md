...
구현된 것: 도메인 로직(지오펜스 판정·오디오 경로·광고 빈도) · Drift 저장소 · 권한 온보딩 · 알림 발화/해제 + 알림 화면 · 전면광고 통합 · 위치 목록/등록/편집 · 플랫폼 권한 선언 · **백그라운드 감시** · **지도 홈·위치 선택 화면** · **알림음** · **진동 세기 조절** · **장소별 알림음·사용자 음원 등록** · **알림 화면 지도 카드** · **Pretendard 폰트**. 테스트 496건.

**알림 화면 가운데에 장소 지도가 있다 (#142·#144, 결정 033).** 지도 카드 400px, 해제 버튼은 340px 에서 **84px 로 줄였다.** 그 버튼은 버스에서 화면을 보지 않고 엄지로 누르는 용도라, **보이는 크기와 눌리는 범위를 분리했다** — 주변 여백까지 탭을 받는다. 더 줄이자는 변경이 오면 `alert_screen_map_test.dart` 가 막는다. 지도는 조작을 받지 않고 lite mode 로 그린다 (전력). **좌표는 `PendingAlert` → SharedPreferences → `take()` 경계를 건너오며, 반드시 `_clear` 앞에서 읽는다** (#125 가 났던 자리).
...
| **#106 진단 기록 표시** | **에뮬레이터 검증 완료** — 실기기에서 기록이 보이는지만 확인하면 된다 |
| **#121 장소별 알림음 검증** | 실기기 필요 — 파일 선택기·백그라운드에서 커스텀 음원 재생·**v2→v3 마이그레이션**. 마이그레이션 테스트가 레포에 하나도 없어 실기기로만 닫힌다 |
| **#142 알림 화면 지도 카드 검증** | 실기기 필요 — **해제 버튼이 84px 로 줄었다.** 급할 때 화면을 안 보고 눌러도 빗나가지 않는지가 핵심 |
| **#145 긴 장소명 확인** | 실기기 필요 — Pretendard 는 글자 폭이 넓다. 48px 장소명이 6자에 한 줄이 꽉 찬다 |
| **#150 스플래시 로고 검증** | 실기기 필요 — Android 12+ 는 스플래시 아이콘에 **원형 마스크**를 씌운다. 핀이 잘리지 않는지, 앱 첫 화면으로 넘어갈 때 색이 튀지 않는지 |
| **#143 지도 스타일 재발 여부** | **관찰 대기** — 근본 원인을 못 찾았다. 재발하면 `[map]` 로그에 화면과 사유가 남는다 |
| **#133 위치 튐 임계 검증** | **관찰 대기** — 시속 300km 임계가 적절한지는 실사용 로그가 쌓여야 안다. 로그에서 `사유=위치튐` 빈도를 본다 |
| **#134 앱이 죽는 원인** | **관찰 대기** — #130 으로 피해는 막았으나 왜 죽는지는 모른다. `getHistoricalProcessExitReasons` 로 직전 종료 사유를 읽어야 갈린다 |
| **#129·#130·#131 실기기 검증** | 실기기 필요 — **스피커 유출 없는지**(usage 를 건드렸다 되돌렸다), 넷플릭스가 멈추는지, 앱 강제 종료 후 끄기 버튼이 듣는지 |
| **#121 프리셋 음원 확보** | **사용자 작업** — CC0/PD 음원만. `SoundPreset` enum 한 줄 + pubspec 한 줄이면 늘어난다. 출처는 `assets/sounds/ATTRIBUTION.md` 에 기록 |
...
**새 코드를 쓸 때 로그를 함께 남긴다** (2026-08-20 추가, 이슈 #106). 이 앱은 백그라운드 동작이 핵심이라 **재현이 안 되고, 남은 기록이 유일한 단서다.** "나중에 필요하면 추가한다"는 그 시점에 이미 늦다 — 사용자가 겪은 그 순간은 지나갔다.

아래 지점은 **로그 없이 머지하지 않는다**:

| 지점 | 남길 것 |
|---|---|
| 알림 발화·해제 | 장소·방향·진동 세기·소리 여부 |
| 오디오 판정 | 이어폰 연결 여부와 장소 설정, 그 결과 |
| 권한 상태 조회 | 여섯 권한의 값 전부 |
| 장소 추가·수정·삭제·토글 | 이름·좌표·반경·방향 |
| 앱 생명주기 전환 | resumed/paused 등 |
| 지오펜스 등록·동기화 | 요청 건수와 실제 등록 건수 |
| 백그라운드 판정 | 좌표·정확도·판정 결과와 **왜 안 울렸는지 사유** (이슈 #127) |
| 상태 전이 | `outside → inside` 처럼 전이 자체 — 알림 여부와 별개다 |
| isolate·프로세스 경계 | 넘길 때와 받을 때 **양쪽** (이슈 #125 가 그래서 안 보였다) |
| 예외를 삼키는 모든 자리 | 사유 |

**진단 화면은 파일을 직접 읽는다** (이슈 #106). `Diagnostics.logger` 를 거치면 초기화 실패 시 파일에 기록이 있어도 "0건"으로 보인다. 진단 대상의 초기화 성공에 의존하는 진단 화면은 정확히 그것이 실패했을 때 쓸모가 없다.
→ `docs/04-CONVENTIONS.md`
