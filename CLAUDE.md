...
구현된 것: 도메인 로직(지오펜스 판정·오디오 경로·광고 빈도) · Drift 저장소 · 권한 온보딩 · 알림 발화/해제 + 알림 화면 · 전면광고 통합 · 위치 목록/등록/편집 · 플랫폼 권한 선언 · **백그라운드 감시(#63, native_geofence — OS 위임)** · **지도 홈·위치 선택 화면** · **알림음**. 테스트 136건.
...
| 작업 | 막힌 이유 |
|---|---|
| **#74 백그라운드 알림 지속 발화 검증** | 실기기 필요 — **코드는 들어갔으나 하나도 확인되지 않았다.** 다른 검증보다 먼저 |
