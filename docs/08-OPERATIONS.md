...
### 근본 해결은 템플릿 쪽이다 — [projectops#545](https://github.com/Cassiiopeia/projectops/issues/545)

워크플로우는 projectops 템플릿이 관리하므로 이 저장소에서 고치면 다음 템플릿 갱신에 덮인다. 상류에 이슈를 올렸다 — 게이트를 커밋 메시지가 아니라 **`version.yml` 의 버전에 해당하는 태그가 있는가**로 판정하면 순서와 무관해지고 재실행에도 안전하다.
